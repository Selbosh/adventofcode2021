...
#' @rdname day17
#' @param input A string of the form `target area: x=20..30, y=-10..-5`
#' @export
read_target <- function(input) {
  target <- strsplit(input, '[^[0-9\\-]')[[1]]
  setNames(as.numeric(target[nchar(target) > 0]),
                     c('x1', 'x2', 'y1', 'y2'))
}
...
#' @rdname day16
#' @param target A vector of length 4 containing the bounds of the target box.
#' @param trick Measure height of trick shot? If `FALSE`, count all possible trajectories.
#' @export
